...
HRESULT CCookMiddleFish::Ready_GameObject()
{
	...
	m_tInfo = { CONSUM,
		COOK_MIDDLEFISH,
		L"생선 꼬치",
		L"생선을 구워서 꼬치에 꽂았다. 체력이 5만큼 공격력은 10초간 상승한다.",
		300, 1 };
...
void CCookMiddleFish::Use_Item()
{
	...
	//m_pPlayer->SetPlayerSpeed(10); 
	//m_pPlayer->SetPlayerPower(10); 
	m_tInfo.iItemCount--;
}
...
HRESULT CCookMiddleFish::Add_Component()
{
	...
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_DYNAMIC].insert({ L"Com_TransformCookMiddleFish", pComponent });
