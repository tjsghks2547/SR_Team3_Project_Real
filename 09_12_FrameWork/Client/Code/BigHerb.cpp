...
CBigHerb::CBigHerb(LPDIRECT3DDEVICE9 pGraphicDev)
    :CItem(pGraphicDev)
{
}
...
HRESULT CBigHerb::Ready_GameObject()
{
...
_int CBigHerb::Update_GameObject(const _float& fTimeDelta)
{
	return CItem::Update_GameObject(fTimeDelta);
}

void CBigHerb::LateUpdate_GameObject(const _float& fTimeDelta)
{
	CItem::LateUpdate_GameObject(fTimeDelta);
}

void CBigHerb::Render_GameObject()
{
...
HRESULT CBigHerb::Add_Component()
{
	...
	//SmallFruit
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_BigHerb"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Com_TextureBigHerb", pComponent });

	pComponent = m_pTransformCom = dynamic_cast<CTransform*>(Engine::Clone_Proto(L"Proto_Transform"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_DYNAMIC].insert({ L"Com_TransformBigHerb", pComponent });
...
CBigHerb* CBigHerb::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CBigHerb* pItem = new CBigHerb(pGraphicDev);

	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("BigHerb Create Failed");
		return nullptr;
	}
